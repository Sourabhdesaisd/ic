...
input 	[7:0]	soc_mmr_write_data_i	    ,//from core
input 		    soc_mmr_read_en_i		    ,//from core
input 	[15:0]	soc_mmr_read_addr_i		    ,//from core
output 	[7:0]	soc_mmr_read_data_o		    ,//to core
...
wire [15:0] 	irq0_ctrl_w 			    ; 
wire [15:0] 	irq1_ctrl_w 			    ; 
wire [15:0] 	irq2_ctrl_w 			    ; 
wire [15:0] 	irq3_ctrl_w 			    ; 
wire [15:0] 	irq4_ctrl_w 			    ; 
wire [15:0] 	irq5_ctrl_w 			    ; 
wire [15:0] 	irq6_ctrl_w 			    ; 
wire [15:0] 	irq7_ctrl_w 			    ; 
wire [15:0] 	irq8_ctrl_w 			    ; 
wire [15:0] 	irq9_ctrl_w 			    ; 
wire [15:0] 	irq10_ctrl_w			    ; 
wire [15:0] 	irq11_ctrl_w			    ;
wire [15:0] 	irq12_ctrl_w			    ;
wire [15:0] 	irq13_ctrl_w			    ;
wire [15:0] 	irq14_ctrl_w			    ;
wire [15:0] 	irq15_ctrl_w			    ;
...
//reg read_en_sync3_r;
reg [7:0]        final_sync_write_data_r;
reg [15:0]        final_sync_write_addr_r;
reg               final_sync_write_en_r  ;
...
if(!soc_rst) begin
    ...
    //read_en_sync3_r    <= read_en_sync2_r;
    end
end

//--------------- MMR WRITE SYNC -------------------------
//reg [7:0] sync_write_data_w;
...
//    end
//else begin
//    sync_write_data_w  = final_sync_write_data_r                       ;
//    sync_write_addr_w  = final_sync_write_addr_r                       ;
//    sync_write_en_w    = final_sync_write_en_r                         ;
//    end
//end
...
always@ (posedge soc_clk or negedge soc_rst) begin
    if (!soc_rst) begin
        final_sync_write_data_r  <= 8'd0                        ;
        final_sync_write_addr_r  <= 16'd0                       ;
        final_sync_write_en_r    <= 1'b0                        ;
    end
    else if (write_en_sync2_r) begin
        final_sync_write_data_r  <= soc_mmr_write_data_i                       ;
        final_sync_write_addr_r  <= soc_mmr_write_addr_i                       ;
        final_sync_write_en_r    <= write_en_sync2_r                           ;
    end
end
...
//reg [7:0]  read_data_hold_r;
reg         read_rsp_toggle_r;
//
always@ (posedge soc_clk or negedge soc_rst) begin
    if (!soc_rst) begin
        //read_data_hold_r <= 8'd0;
  ...
  reg [7:0]  final_sync_read_data_r;
  reg        final_sync_read_rsp_r ;

always@ (posedge soc_clk or negedge soc_rst) begin
if (!soc_rst) begin
    final_sync_read_data_r   <= 8'd0          ;
    final_sync_read_rsp_r    <= 1'b0          ;
    end
else begin
    final_sync_read_data_r  <=   soc_mmr_read_data_w                         ;
    final_sync_read_rsp_r   <=   read_rsp_toggle_r                           ;
    end
end


assign soc_mmr_read_data_o =    final_sync_read_data_r      ;
assign soc_read_rsp_o      =    final_sync_read_rsp_r       ;
...
.soc_mmr_write_en_i		        (final_sync_write_en_r		),//from core
.soc_mmr_write_addr_i		    (final_sync_write_addr_r    ),//from core
.soc_mmr_write_data_i		    (final_sync_write_data_r	),//from core
.soc_mmr_read_en_i		        (read_en_sync2_r		        ),//from core
...
.soc_eoi_valid_i	(soc_eoi_valid_i			),//end of interrupt valid signal from core/soc_mmr
...
always@(posedge soc_clk or negedge soc_rst) begin
    ...
    else if (final_sync_write_en_r) begin
        trace_event_int_o   <= MMR_WRITE;
        trace_data_int_o    <= {8'd0,final_sync_write_data_r[7:0],final_sync_write_addr_r};
    end
