...
pend_exp_lvl_pr = 8'h00;
  ...
  function automatic bit higher_priority(
    ...
    // tie-breaker: higher interrupt ID wins
    if ((cur_ctl[7:5] == best_ctl[7:5]) &&
        (cur_ctl[4:2] == best_ctl[4:2]) &&
        (cur_id > best_id))
      return 1'b1;
  ...
  tr.zic_rst = vif.zic_rst;
    ...
    // IRQ47 is WDT/internal, so external TB uses only 0 to 46
  ...
  if(best_found &&
     (best_ctl > tr.active_lvl_pr_i)) begin
...
// Case 3: delayed ACK output became valid
  ...
  `uvm_info(
      "MON_PREDICT",
      $sformatf(
      "best_id=%0d exp_ack=0x%0h ctl=0x%0h level=0x%0h",
      best_id,
      tr.exp_ack_id,
      best_ctl,
      tr.exp_highest_lvl_pr
