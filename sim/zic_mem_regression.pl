################# ZIC MEM MAP REGRESSION SCRIPT FOR ALL TESTS ###############################

##Command to run this scrpit with coverage : "perl zic_mem_regression.pl <test_list.f> <1>"
...
##Command to run this scrpit without coverage : "perl zic_mem_regression.pl <test_list.f>"
# test_list.f : Create a file named test_list.f and inside that write all test names and number for how many times those tests should run.
#                eg: zic_mem_test 100
...
sub simulate_test 
{
  ...
  my ($test) = @_;
   {   
    ...
    {  
      system("xrun -access +rwc -f ../UVME/zic_mem_comp_file.f  -svseed $seed +UVM_TESTNAME=$test +define+UVM_REPORT_DISABLE_FILE_LINE -uvmhome CDNS-1.1d"); 
    }
