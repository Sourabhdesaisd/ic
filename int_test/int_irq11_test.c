...
#define EXP_IRQ11_CTL_VALUE       0x000000D4U
...
int main(void)
{
    ...
    uint32_t actual_irq11_enable;
    uint32_t actual_irq11_attr;
    uint32_t actual_irq11_ctl;
    ...
    /* Read back IRQ11 ENABLE register */

    actual_irq11_enable =
        mmio_read(IRQ11_ENABLE_REG_ADDR);


    if (actual_irq11_enable != EXP_IRQ11_ENABLE_VALUE)
        error_print(2);
    else
        info_print(3);
    ...
    /* Read back IRQ11 CONTROL register */

    actual_irq11_ctl =
        mmio_read(IRQ11_CTL_REG_ADDR);


    if (actual_irq11_ctl != EXP_IRQ11_CTL_VALUE)
        error_print(4);
    else
        info_print(5);
     ...
     * Inform SV/UVM
     *
     * After this handshake, SV/UVM should generate the
     * GPIO interrupt corresponding to IRQ11.
     * ============================================================ */
