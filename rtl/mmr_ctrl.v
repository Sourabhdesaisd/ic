//ctl register which has ip,ie,attr,ctrl sub registers
//ip--interrupt pending
//ie--interrupt enable
//attr--level/edge 
//ctrl--lvl/pr of interrupts
module zic_mmr_ctrl
(
input 		zic_clk			,
input 		zic_rst			,//active low reset
input [15:0] 	zic_mmr_write_addr	,//addr to write into mmr
input [31:0] 	zic_mmr_write_data	,//data that get write into mmr
input 	     	zic_mmr_write_en	,//enable to write to mmr
input [47:0] 	zic_int_pending_bit	,//pending vectore from ic
input           zic_int_pending_valid	,//pending valid bit from ic
input [47:0]	global_int_enable_bit	,//global enable vector from tb
input           global_int_enable_valid ,//global valid from tb
output reg [47:0] 	zic_int_en_o		,//interrupts enable vectore to ic
output [31:0] 	irq0_ctrl_o		,//interrupts lvl/pr values to ic
...
always@(posedge zic_clk or negedge zic_rst)begin
	if(!zic_rst)begin
		zic_int_en_o<=48'b0;
	end
	else begin
...
begin
	...
	else if(zic_mmr_write_en && (zic_mmr_write_addr == MMR_CTL_ADDR))begin
