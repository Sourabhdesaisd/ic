//interrupt contrller memory mapped register top module
module zic_mmr_top
(
input 		zic_clk			,
input 		zic_rst			,
...
input 		zic_ack_write_valid_i		,//ack int id write enable from ic
input   [7:0] 	zic_ack_int_id_i		,//ack int id from ic
input 		zic_ack_read_valid_en		,//ack int id read en from processor
output 	[7:0]	zic_ack_int_id_o		,//to processor ack
output 		zic_ack_o			,//acknowledgement signal to ic
...
output 		zic_eoi_o			,//eoi signal to ic
//next pending interrupt id signals
input 		zic_nxtp_valid_i		,//from ic to reg file
input  [7:0] 	zic_nxtp_id_i			,//from ic to reg file
//interrupt pending signals
input 		zic_int_pending_valid_i 	,// make one bit	
input [47:0] 	zic_int_pending_bit_i		,
//interrupt enable signals
input 	[47:0]	global_int_enable_bit_i		,
input  	  		global_int_enable_valid_i	 //make one bit
);
...
.zic_int_pending_bit_i		(zic_int_pending_bit_i		),//ic interrupt pending bits
.global_int_enable_bit_i	(global_int_enable_bit_i	),//ic interrupt enable signal
.global_int_enable_valid_i	(global_int_enable_valid_i	),
.zic_ack_valid_i		(zic_ack_write_valid_i		),//from ic to regfile
.zic_ack_int_id_i		(zic_ack_int_id_i		),//from ic to regfile
.zic_ack_int_id_o		(ack_int_id_w			),//to read mux 
.zic_nxtp_valid_i		(zic_nxtp_valid_i		),//from ic to reg file	
.zic_nxtp_id_i			(zic_nxtp_id_i			),//from ic to reg file
.zic_eoi_valid_i		(zic_eoi_valid_i		),//to reg file
.zic_eoi_id_i			(zic_eoi_id_i			),//to reg file
.zic_eoi_o			(zic_eoi_id_w			),//to read mux 
.zic_cfg_o			(zic_cfg_w			),//to read mux and to ic
.zic_info_o			(zic_info_w			),//to read mux
.zic_nxtp_o			(zic_nxtp_w			),//to read mux
.irq0_ctrl_o 			(irq0_ctrl_w			),//to ic and read mux		
...
.zic_int_en_o           (zic_int_en_o)//to ic
);
...
.zic_ack_read_valid	(zic_ack_read_valid_en	),
